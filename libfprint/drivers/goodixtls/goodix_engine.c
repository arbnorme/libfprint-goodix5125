 ...
 *
 * Adapted for goodixtls5125 (Goodix 27c6:5125) from goodix-5e0a
 * (https://github.com/jitendradara12/goodix-5e0a, libfprint-driver/goodix_milan.c):
 * loads Goodix's AlgoChicago.dll from the user's own Windows driver package
 * (fetched by scripts/goodix5125-fetch-engine; never distributed here), adds
 * the extra Win32 shims it imports, and reports load failures as GError.
 *
 * Implements native in-process PE loader for Goodix engine DLLs on Linux.
...
fail:;
    ...
    g_warning("goodix5125-engine: %s: %s failed for %s: errno=%d (%s)%s",
...
__attribute__((constructor)) static void goodix_engine_mutex_init(void) {
    g_rec_mutex_init(&g_milan_mutex);
}

gboolean goodix_engine_init (const char *dll_path, GError **error) {
    ...
    if (!dll_path || !*dll_path) {
        g_set_error (error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "No Goodix engine DLL path given");
        g_rec_mutex_unlock (&g_milan_mutex);
        return FALSE;
    }

    if (load_pe_file(dll_path) != 0) {
        int e = errno;
        g_set_error (error, G_IO_ERROR, g_io_error_from_errno (e),
                     "Could not load the Goodix engine '%s': %s%s", dll_path, g_strerror (e),
                     e == ENOENT ? " (run goodix5125-fetch-engine to install it)" :
                     (e == EACCES || e == EPERM) ? " (SELinux may be blocking the in-process engine; see packaging/selinux)" : "");
        g_rec_mutex_unlock (&g_milan_mutex);
        return FALSE;
    }
    ...
    if (!m_getAlgorithmVersion || !m_ppp_param_init || !m_enrolStartEx ||
        ...
        g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                     "'%s' is not a supported Goodix engine (required exports missing)", dll_path);
    ...
    if (!m_getQuality)
        g_debug ("goodix5125-engine: Milan getQuality export missing; frame judging falls back to residual range and active area");

    ensure_gs();
    m_getAlgorithmVersion(g_milan_version);
    m_ppp_param_init(10); /* 64x80 sensor initialization */

    g_message("goodix5125-engine: Milan biometric matching engine loaded successfully (%s)", g_milan_version);
    g_milan_available = TRUE;
    g_rec_mutex_unlock (&g_milan_mutex);
    return TRUE;
}

const char *goodix_engine_get_version (void) {
...
guint goodix_engine_frame_quality (const uint8_t *pixels,
                                  int width,
                                  int height,
                                  guint *out_quality,
                                  guint *out_overlap) {
...
void *goodix_engine_enroll_start (int *max_images) {
    if (!g_milan_available) return NULL;
    g_rec_mutex_lock (&g_milan_mutex);
    ensure_gs();
    int max_imgs = 16;
...
int goodix_engine_enroll_add_image (void *ctx,
...
int goodix_engine_enroll_commit (void *ctx,
                                uint8_t **out_blob,
                                size_t *out_len) {
...
void goodix_engine_enroll_finish (void *ctx) {
    if (!ctx) return;
    g_rec_mutex_lock (&g_milan_mutex);
    ensure_gs();
    m_enrolFinish(ctx);
...
int goodix_engine_verify_image (const uint8_t *pixels,
                               int width,
                               int height,
    ...
    if (unpack_res != 0 || !unpacked_template) {
        g_warning("goodix5125-engine: templateUnPack failed (err=%d)", unpack_res);
        g_rec_mutex_unlock (&g_milan_mutex);
        return 0;
    }
...
int goodix_engine_identify_image (const uint8_t *pixels,
                                 int width,
                                 int height,
...
fail_closed:
    g_warning ("goodix5125-engine: gallery templateUnPack failed; rejecting without match");
    for (int i = 0; i < n_templates; i++)
        if (unpacked[i]) m_templateDelete (unpacked[i]);
    g_free (unpacked);
    g_rec_mutex_unlock (&g_milan_mutex);
